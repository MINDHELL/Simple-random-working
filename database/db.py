...
async def add_user(user_id: int, username: str):
    try:
        await db.users.insert_one({"user_id": user_id, "username": username, "plan": "free"})
...
async def get_all_users():
    return await db.users.distinct("user_id")
...
async def get_user_plan(user_id: int):
    ...
    return user.get("plan") if user else "free"
...
async def is_premium(user_id: int):
    return (await get_user_plan(user_id)) == "premium"
